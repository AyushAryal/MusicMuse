...
def chord2vec(sentences, min_count=1, context_window=3, vector_size=10, algorithm=0, epochs=5, workers=3, callbacks=[]):
    model = Word2Vec(sentences, min_count=min_count, window=context_window, vector_size=vector_size, sg=algorithm, epochs=epochs, workers=workers, callbacks=callbacks)
    return model
